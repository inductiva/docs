...
> Windows users might experience an error when Python is installed but pip is either missing or not added to your system PATH. If that's the case, you might get the error:
```python
>>>pip install inductiva
...
You can bypass that by running the following from a regular Command Prompt (not inside Python):
```python
python -m pip install --upgrade pip
python -m pip install inductiva
```
