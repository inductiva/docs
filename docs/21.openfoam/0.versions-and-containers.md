...
description: Explore supported OpenFOAM versions and container images on the Inductiva.AI Cloud HPC platform.
seo:
 title: OpenFOAM Versions and Docker Containers on Inductiva.AI
 description: Explore supported OpenFOAM versions and container images on the Inductiva.AI Cloud HPC platform.
---
...
**OpenFOAM (ESI)**
- **25.06** (Jul., 2025)
- **24.12** (Dec., 2024) – includes **cfMesh** solver
- **24.06** (Jun., 2024)
- **22.06** (Jun., 2022)
...
All supported versions come with the standard OpenFOAM solvers pre-installed. Additionally, OpenFOAM (ESI) version 24.12 includes the cfMesh solver.
...
To use the cfMesh solver, add the `include_cfmesh` flag when initializing the simulator:

```python
# Initialize the Simulator with include_cfmesh flag
OpenFOAM = inductiva.simulators.OpenFOAM(
    version="24.12",
    distribution="esi",
    include_cfmesh=True
